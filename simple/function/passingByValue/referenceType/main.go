...
/*
 ...
 Back to  application. The built-in function map(T, args)
serves a purpose different from new(T), It creates slices, maps, and channels only,
and it returns an initialized (not zeroed) value of type T (not *T). The reason for the
distinction is that these three types represent, under the covers, references to data structures
